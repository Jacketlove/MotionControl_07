...
public class ControlBoard {
	...
	public void configControlBoard(){
		motionGetCardType();
		//定义轴数量
		this.axias = new Axia[controlBoardTypeEnum.getMaxAxisNum()];
		//实例化轴
		for(int i=0; i<controlBoardTypeEnum.getMaxAxisNum(); i++){
			this.axias[i] = new Axia(i);
		}
	}
	...
	public void configEMGPin(int enable, int emg_logic){
		JNAInvokeDll.motionDrvDll.motion_config_EMG_PIN(this.cardno, enable, emg_logic);
	}
	...
	public void writeOutbit(int bitno, int on_off){
		JNAInvokeDll.motionDrvDll.motion_write_outbit(this.cardno, bitno, on_off);
	}
	...
	public void readOutbit(int bitno){
		JNAInvokeDll.motionDrvDll.motion_read_outbit(this.cardno, bitno);
	}
	...
	public int readInport(){
		int temp = JNAInvokeDll.motionDrvDll.motion_read_inport(this.cardno);
	...
	public void motionTLine4(int Dist1, int Dist2, int Dist3, int Dist4, int posi_mode){
		JNAInvokeDll.motionDrvDll.motion_t_line4(this.cardno, Dist1, Dist2, Dist3, Dist4, posi_mode);
	}
	...
	public void motionTrigerChunnel(int num){
		JNAInvokeDll.motionDrvDll.motion_triger_chunnel(this.cardno, num);
	}
	...
	public void motionGetCardType(){
		int temp;
		temp = JNAInvokeDll.motionDrvDll.motion_get_card_type(cardno);
